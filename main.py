...
def handle_order(msg):
    ...
    if st == "order_name":
        tmp[uid]    = {"name": text}
        states[uid] = "order_deadline"
        send(uid, tr(uid,
            "📅 Qachonga tayyor bo'lishi kerak?\n"
            "(Masalan: 3 kun ichida, 20.06.2024, shoshilinch)",
            "📅 К какому сроку нужно?\n"
            "(Например: через 3 дня, срочно)",
            "📅 When do you need it?\n"
            "(Example: in 3 days, urgent)"
        ))

    elif st == "order_deadline":
        tmp[uid]["deadline"] = text
        states[uid] = "order_desc"
        send(uid, tr(uid,
            "📝 Qanday bot kerakligini batafsil yozing:\n\n"
            "Masalan:\n"
            "— Qanday funksiyalar kerak\n"
            "— Qaysi platformada (Telegram, Instagram...)\n"
            "— Qo'shimcha talablar",
            "📝 Опишите подробно, какой бот нужен:\n\n"
            "Например:\n"
            "— Какие функции нужны\n"
            "— Платформа (Telegram, Instagram...)\n"
            "— Дополнительные требования",
            "📝 Describe in detail what bot you need:\n\n"
            "For example:\n"
        ...
        send(uid, tr(uid,
            "✅ So'rovingiz qabul qilindi! #" + str(order_id) + "\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            "Ism: " + clean(order["name"]) + "\n"
            "Muddat: " + clean(order["deadline"]) + "\n"
            "Tel: " + clean(order["phone"]) + "\n\n"
            "Tez orada bog'lanamiz!",
            "✅ Заявка принята! #" + str(order_id) + "\nСкоро свяжемся.",
            "✅ Request accepted! #" + str(order_id) + "\nWe'll contact you soon."
        ), kb_user_main(uid))

        notify_admin(msg.from_user,
            "📦 Yangi so'rov #" + str(order_id) + "\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            "Ism: " + clean(order["name"]) + "\n"
            "Muddat: " + clean(order["deadline"]) + "\n"
            "Tel: " + clean(order["phone"]) + "\n\n"
