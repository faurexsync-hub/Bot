...
def notify_admin(user, xabar):
    try:
        name = clean(((user.first_name or "") + " " + (user.last_name or "")).strip()) or "Nomsiz"
        un   = "@" + clean(user.username) if user.username else "username yoq"
        bot.send_message(
            ADMIN_ID,
            "📨 Yangi xabar!\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
...
# ─────────────────────────────────────
#  STIKER, RASM, VIDEO — HAMMASI UCHUN
# ─────────────────────────────────────
...
def handle_media(msg):
    ...
    if tur == "sticker":
        ...
        xabar = "Ovozli xabar yubordi"
    elif tur == "audio":
        xabar = "Audio yubordi"
    elif tur == "document":
        xabar = "Fayl yubordi"
    elif tur == "location":
        xabar = "Joylashuv yubordi"
    elif tur == "contact":
        xabar = "Kontakt yubordi"
    else:
        xabar = "Xabar yubordi"

    notify_admin(msg.from_user, xabar)
...
def handle_text(msg):
    ...
    # ── Admin — state yoq, hech narsa qilma
    if is_admin(uid):
        return
    ...
    notify_admin(msg.from_user, clean(text))
...
if __name__ == "__main__":
    print("FUZZY BOT v8.0 ishga tushdi!")
